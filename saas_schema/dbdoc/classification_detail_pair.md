...
ä»•è¨³æ˜Žç´°ãƒšã‚¢. è¨ˆä¸Šæ—¥ãŒåŒã˜å€Ÿæ–¹ã¨å€Ÿæ–¹ã‚’ãƒšã‚¢ã«ã—ã¦æ‘˜è¦ã‚’åŠ ãˆã‚‹ã®ã¯ã€ç”»é¢ç‰¹æœ‰ã®æ¦‚å¿µã¨æ€ã‚ã‚Œã‚‹ã®ã§åˆ¥ãƒ†ãƒ¼ãƒ–ãƒ«ã§è¡¨ç¾ã™ã‚‹ã€‚
...
CREATE TABLE `classification_detail_pair` (
  ...
  PRIMARY KEY (`id`),
  KEY `debit_detail` (`debit_detail`),
  KEY `credit_detail` (`credit_detail`),
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin COMMENT='ä»•è¨³æ˜Žç´°ãƒšã‚¢. è¨ˆä¸Šæ—¥ãŒåŒã˜å€Ÿæ–¹ã¨å€Ÿæ–¹ã‚’ãƒšã‚¢ã«ã—ã¦æ‘˜è¦ã‚’åŠ ãˆã‚‹ã®ã¯ã€ç”»é¢ç‰¹æœ‰ã®æ¦‚å¿µã¨æ€ã‚ã‚Œã‚‹ã®ã§åˆ¥ãƒ†ãƒ¼ãƒ–ãƒ«ã§è¡¨ç¾ã™ã‚‹ã€‚'
```
...
| Name | Definition |
| ---- | ---------- |
| credit_detail | KEY credit_detail (credit_detail) USING BTREE |
| debit_detail | KEY debit_detail (debit_detail) USING BTREE |
