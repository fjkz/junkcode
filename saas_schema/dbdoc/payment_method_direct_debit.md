...
CREATE TABLE `payment_method_direct_debit` (
  ...
  PRIMARY KEY (`id`),
  KEY `tenant` (`tenant`),
  CONSTRAINT `payment_method_direct_debit_ibfk_1` FOREIGN KEY (`tenant`) REFERENCES `tenant` (`id`)
...
| ---- | ---- | ---------- |
| payment_method_direct_debit_ibfk_1 | FOREIGN KEY | FOREIGN KEY (tenant) REFERENCES tenant (id) |
