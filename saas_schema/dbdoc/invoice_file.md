...
CREATE TABLE `invoice_file` (
  ...
  `process_id` int(11) unsigned DEFAULT NULL COMMENT 'ã“ã®è«‹æ±‚æ›¸ãŒå‡¦ç†ã•ã‚Œã‚‹ãƒ—ãƒ­ã‚»ã‚¹ã®ID. è«‹æ±‚æ›¸ã‚’ã‚¢ãƒƒãƒ—ãƒ­ãƒ¼ãƒ‰ã—ã¦ã‹ã‚‰ã€é¸æŠžã—ã¦å‡¦ç†ã™ã‚‹ã¨æŽ¨æ¸¬ã—ã¦ nullable ã«ã™ã‚‹ã€‚',
  PRIMARY KEY (`id`),
  KEY `tenant` (`tenant`),
  KEY `process_id` (`process_id`),
  CONSTRAINT `invoice_file_ibfk_1` FOREIGN KEY (`tenant`) REFERENCES `tenant` (`id`),
  CONSTRAINT `invoice_file_ibfk_2` FOREIGN KEY (`process_id`) REFERENCES `invoice_process` (`id`)
...
| invoice_file_ibfk_2 | FOREIGN KEY | FOREIGN KEY (process_id) REFERENCES invoice_process (id) |
...
| Name | Definition |
| ---- | ---------- |
| process_id | KEY process_id (process_id) USING BTREE |
