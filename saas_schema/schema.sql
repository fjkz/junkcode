...
-- スキーマで表現するために、なるべく外部キー制約を貼っていますが、設計上制約がないところもあります。
...
CREATE TABLE user (
  id int(11) unsigned NOT NULL AUTO_INCREMENT COMMENT 'ID',
  tenant int(11) unsigned NOT NULL COMMENT 'テナント。ユーザーは１つのテナントに所属し、テナントの付け換えもないものとする。',
...
ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin
COMMENT='銀行振り込みによる支払方法. 支払先ではなく、支払い元の銀行口座を示している。支払い方法のカテゴリごとにテーブルを分ける。外部キー制約はつけられないが、ほとんどのフィールドを使わなくて NULL ばかりにならないことを優先する。';
...
CREATE TABLE account_category (
  ...
  tenant int(11) unsigned NOT NULL COMMENT 'テナント',
  name varchar(255) NOT NULL COMMENT '表示名',
  dr_cr_class enum('debit', 'credit') NOT NULL COMMENT '借方 or 貸方',
  PRIMARY KEY (id),
  FOREIGN KEY (tenant) REFERENCES tenant (id)
)
ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin
COMMENT='科目。科目は独自に追加できるとして、テナント毎に科目を用意する。';
...
ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin
COMMENT='税区分. 税率変更を考慮して、ENUM 型で表現せずに別テーブルにする。税区分はテナントによらない。';
...
CREATE TABLE classification_detail (
  ...
  amount int(11) NOT NULL COMMENT '金額. 請求を債権と相殺するなどマイナスはありうるので、signed とする。',
...
CREATE TABLE classification_detail_pair (
  id int(11) unsigned NOT NULL AUTO_INCREMENT COMMENT 'ID',
  debit_detail int(11) unsigned COMMENT '借方明細。貸方明細と同じ classification に属する。',
  credit_detail int(11) unsigned COMMENT '貸方明細。貸方明細と同じ classification に属する。',
...
CREATE TABLE invoice_process (
  ...
  payment_method_type enum("bank_transfer", "direct_debit", "credit card") NOT NULL COMMENT '支払い方法種別',
  payment_method_id int(11) unsigned NOT NULL COMMENT '支払い方法。payment_method_type が示すテーブルのID。外部キー制約はつけられない。',
...
ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin
COMMENT='請求書処理。請求書の処理と支払いは一貫して行われるものと想定してまとめている。';
...
CREATE TABLE invoice_file (
  ...
  process_id int(11) unsigned COMMENT 'この請求書が処理されるプロセスのID. 請求書をアップロードしてから、選択して処理すると推測して nullable にする。',
  PRIMARY KEY (id),
  FOREIGN KEY (tenant) REFERENCES tenant (id),
  FOREIGN KEY (process_id) REFERENCES invoice_process (id)
)
ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin
