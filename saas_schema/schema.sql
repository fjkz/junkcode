...
ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin
COMMENT='取引先の銀行口座。payment_method_bank_transfer とフィールドは重複するが、共通部分を切り出すと余計なテーブルが増えてクエリが複雑になるので、重複したフィールドをそれぞれに持たせている。';
...
ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin
COMMENT='銀行振り込みによる支払方法。支払先ではなく、支払い元の銀行口座を示している。支払い方法のカテゴリごとにテーブルを分ける。この結果外部キー制約はつけられない。一方で、すべての支払い種別を一つのテーブルで表現すると、フィールドがNULLばかりになるのと、支払い種別の追加が難しくなる。';
...
CREATE TABLE invoice_process (
  ...
  FOREIGN KEY (tenant) REFERENCES tenant (id),
  FOREIGN KEY (payee) REFERENCES business_partner (id), -- (payee, tanant) ともできるが、過剰な制約と判断してやらない。(pic) も同じ。
